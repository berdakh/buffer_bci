...
*  [`python`](python) -- example BCIs written in the [Python](http://www.python.org) programming language.
   See [`python/pyspeller`](python/pyspeller) for a self-contained pure-python version of the whole
   framework -- buffer server, acquisition (simulated or over Lab Streaming Layer, e.g. g.tec),
   signal processing, a P300 speller and a control GUI with real-time signal display.
